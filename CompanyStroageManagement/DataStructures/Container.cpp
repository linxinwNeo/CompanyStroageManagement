...
void Container::searchResult(QVector<QString> &items) const
{
    ...
    items.push_back(GlobalVars::locale.toString(this->num_models())); // 货物种类数量

    items.push_back(GlobalVars::locale.toString(this->total_num_init_boxes(), 'f', 2)); // 初始总箱数
    items.push_back(GlobalVars::locale.toString(this->total_num_left_boxes(), 'f', 2)); // 剩余总箱数

    items.push_back(GlobalVars::locale.toString(this->total_num_init_pieces())); // 初始总个数
    items.push_back(GlobalVars::locale.toString(this->total_num_left_pieces())); // 剩余总个数
}
